...
void NATNET_CALLCONV DataHandler(sFrameOfMocapData* data, void* pUserData)
{
	...
	//Openning writting file
	ofstream fichier("/home/julien/Map/output.txt", ios::out | ios::trunc);
	
	// Rigid Bodies
	printf("Rigid Bodies [Count=%d]\n", data->nRigidBodies);
	//Writting Rigid Body Number
	fichier<< data->nRigidBodies <<endl;
	for(i=0; i < data->nRigidBodies; i++)
	{
		...
		fichier<< data->RigidBodies[i].ID<< " " << data->RigidBodies[i].x << " " << data->RigidBodies[i].y << " " << data->RigidBodies[i].z << endl;
	}
fichier.close();
